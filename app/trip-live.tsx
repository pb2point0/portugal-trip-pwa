...
export function WeatherForecast({days}:{days:TripDay[]}){
  ...
  const requested=useMemo(()=>showAll?days:days.slice(0,4),[days,showAll]);
    ...
    async function load(){
      try{
        ...
        const results=await Promise.all(uniquePlaces.map(async(place)=>{
          const params=new URLSearchParams({latitude:String(place.lat),longitude:String(place.lng),timezone:place.timezone,forecast_days:'16',temperature_unit:'fahrenheit',daily:'weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max'});
        ...
        for(const day of requested){
          ...
          if(place&&index>=0&&result?.payload.daily)next[day.date]={date:day.date,place:place.name,code:result.payload.daily.weather_code[index],high:Math.round(result.payload.daily.temperature_2m_max[index]),low:Math.round(result.payload.daily.temperature_2m_min[index]),rain:result.payload.daily.precipitation_probability_max[index]??0};
        }
        if(cancelled)return;
        const loadedCount=Object.keys(next).length;
        setForecast((current)=>{
          const merged={...current,...next};
          localStorage.setItem(WEATHER_CACHE_KEY,JSON.stringify({savedAt:Date.now(),data:merged}));
          return merged;
        });
        setStatus(loadedCount?`Fresh forecast for ${loadedCount} of ${requested.length} shown days · °F`:'Live forecasts populate up to 16 days ahead');
      }catch{
  ...
  return<section className="weather-section" aria-labelledby="weather-title">
    <div className="section-heading"><div><p className="kicker">Live trip weather</p><h2 id="weather-title">Pack for the day, not the season.</h2></div><div className="weather-actions"><small>{status}</small>{days.length>4&&<button type="button" onClick={()=>setShowAll((current)=>!current)}>{showAll?'Show next four':`Whole trip · ${days.length} days`}</button>}</div></div>
    <div className={'weather-grid '+(showAll?'whole-trip':'')}>{requested.map((day)=>{
      ...
      return<article key={day.date} className={!item?'forecast-pending':''}>
        ...
        {item?<><WeatherIcon code={item.code}/><div><strong>{item.high}°F</strong><small>{item.low}°F low</small></div><p>{weatherCopy(item.code)} · {item.rain}% rain</p><b>{item.place}</b></>:<><Cloud size={21}/><div><strong>—</strong><small>Forecast pending</small></div><p>We’ll fill this in automatically when it enters the forecast window</p><b>{day.base}</b></>}
      </article>;
    })}</div>
  </section>;
}
...
export function MobilityPanel({day}:{day:TripDay}){
  return<div className="mobility-panel">
    ...
    <div className="mobility-options">
      <a href={mapsUrl(day.base,'walking')}><Footprints size={18}/><span><b>Walking time</b><small>Open live route</small></span><ExternalLink size={14}/></a>
      <a href={mapsUrl(day.base,'driving')}><CarFront size={18}/><span><b>Driving time</b><small>Includes live traffic</small></span><ExternalLink size={14}/></a>
      <a href="https://bolt.eu/en/cities/"><Navigation size={18}/><span><b>Check Bolt</b><small>Live fare in Bolt</small></span><ExternalLink size={14}/></a>
    </div>
